...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $resultado = $detVentaNegocio->crearDetVenta($datos);
                ...
                <form action="crear.php" method="POST">
                    <div class="row g-3">
                            ...
                            <select id="cabventa" name="IdCabVenta" class="form-select" required>
                                ...
                                <?php else: ?>
                                    <?php foreach ($cabVentas as $cabVenta): ?>
                                        <option value="<?php echo mostrarValor($cabVenta['IdCabVenta']); ?>" <?php echo ($cabVenta['IdCabVenta'] == $datos['IdCabVenta']) ? 'selected' : ''; ?>><?php echo mostrarValor($cabVenta['IdCabVenta'] . ' - ' . ($cabVenta['NombreCliente'] ?? 'Cliente')); ?></option>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </select>
                        </div>
                            ...
                            <select id="producto" name="IdProducto" class="form-select" required>
                                ...
                                <?php else: ?>
                                    <?php foreach ($productos as $producto): ?>
                                        <option value="<?php echo mostrarValor($producto['IdProducto']); ?>" <?php echo ($producto['IdProducto'] == $datos['IdProducto']) ? 'selected' : ''; ?>><?php echo mostrarValor($producto['NombreProducto']); ?></option>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </select>
                        </div>
                        <div class="col-md-3">
                            ...
                            <input type="number" min="1" step="1" class="form-control" id="cantidad" name="Cantidad" value="<?php echo mostrarValor($datos['Cantidad']); ?>" required>
                        </div>
                        <div class="col-md-3">
                            <label for="precio" class="form-label">Precio unitario</label>
                            <input type="number" min="0.01" step="0.01" class="form-control" id="precio" name="PrecioUnitario" value="<?php echo mostrarValor($datos['PrecioUnitario']); ?>" required>
                        </div>
                    </div>
